...
sites_Allsamps <- readRDS('../random forest/rf_site_importance_Allsamps.rds') |> 
  filter(pval <= 0.05) |> 
  pull('loc.site')

# best age and methylation
message(format(Sys.time()), ' : Best - All')
predictAllIDsGAM(train.df, model.df, sites_Allsamps, 'age.best') |> 
  saveRDS('gam_best_Allsamps.rds')
