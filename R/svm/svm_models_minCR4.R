...
pred.ranAge <- parallel::mclapply(1:nrep, function(j) {
  ...
  bind_rows()
saveRDS(pred.ranAge, paste0('R/svm/svm_ranAge_minCR4_', sites.2.use,'_', age.transform, '_', weight, '.rds'))
...
pred.ranAgeMeth <- parallel::mclapply(1:nrep, function(j) {
  ...
  bind_rows()
saveRDS(pred.ranAgeMeth, paste0('R/svm/svm_ranAgeMeth_minCR4_', sites.2.use, '_', age.transform, '_', weight, '.rds'))
